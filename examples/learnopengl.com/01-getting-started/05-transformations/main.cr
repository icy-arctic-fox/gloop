...
# ------------------------------------
our_shader = Shader.new(context, "5.1.transform.vs", "5.1.transform.fs")
...
while LibGLFW.window_should_close(window).false?
  ...
  transform = Geode::Matrix4x4(Float64).identity
  transform = transform.translate(0.5, -0.5, 0.0)
  transform = transform.rotate(LibGLFW.get_time.to_f, Geode::Vector3[0.0, 0.0, 1.0])
  transform = transform.map(&.to_f32)

  # get matrix's uniform location and set matrix
  our_shader.use
  transform_loc = Gloop::Program::Uniforms.new(context, our_shader.id).locate("transform")
  LibGL.uniform_matrix4_fv(transform_loc, 1, LibGL::Boolean::False, transform)
